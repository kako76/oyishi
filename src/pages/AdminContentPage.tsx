...
export const AdminContentPage: React.FC = () => {
  ...
  const [editingProduct, setEditingProduct] = useState<OyishiProduct | null>(null);
  const [isSaving, setIsSaving] = useState(false);
  const [saveError, setSaveError] = useState('');
  const [saveSuccess, setSaveSuccess] = useState(false);

  // Local state to instantly reflect changes
  const [localProducts, setLocalProducts] = useState<OyishiProduct[]>([]);
  ...
  const toggleProductActive = async (product: OyishiProduct) => {
